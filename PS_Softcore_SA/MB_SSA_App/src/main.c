...
#ifdef USE_DISPLAY
    // Init the display
    Status = init_Display_SSD1309(&Display_SSD1309, &AXI_SPI_DisplayHandle, DISPLAY_CSN, XPAR_AXI_QUAD_SPI_0_FIFO_SIZE, displayResetOrRun, displayCommandOrData, displayTrasmitReceive, displayChipSelect, sleep_ms, sleep_10us, &U8G2);
    if (Status == false)
        while(1);
...
static void displayResetOrRun(Type_DisplayResetRun ResetRunAction)
{
    if (ResetRunAction == DISPLAY_RUN)
        XGpio_DiscreteSet(&AXI_GPIO_Handle, GPIO_OUTPUT_CHANNEL, 0x04);
    else
...
static void displayCommandOrData(Type_DisplayCommandData CommandDataAction)
{
    if (CommandDataAction == DISPLAY_DATA)
        XGpio_DiscreteSet(&AXI_GPIO_Handle, GPIO_OUTPUT_CHANNEL, 0x08);
    else
...
{
    if (Status == CS_ENABLE)
        XGpio_DiscreteClear(&AXI_GPIO_Handle, GPIO_OUTPUT_CHANNEL, 0x10);
    else
        XGpio_DiscreteSet(&AXI_GPIO_Handle, GPIO_OUTPUT_CHANNEL, 0x10);
}
